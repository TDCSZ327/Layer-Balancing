...
def parse_args():
    p = argparse.ArgumentParser(description="3-NN (Gaussian init): sweep lr_max and plot diagonal curves for arbitrary steps-list with colorbar")
    ...
    p.add_argument("--steps-list", type=str, default="1,2,4,8", help="Comma-separated steps to record, e.g. '1,2,4,8'")
...
def compute_diagonal_curves_for_one_lrmax(
    ...
    """
    For a given lr_max:
      - define step_val = max(1, lr_max//20) exactly like your code
      - define N = int(lr_max/step_val)
      - diagonal index constraint: i + j = N  -> lr1=i*step_val, lr2=j*step_val
      - compute test loss curves along this diagonal for EACH step in steps_list.

    Returns:
      xs: np.array of eta1 values along diagonal
      losses_by_step: dict {step: np.array of avg test loss along diagonal}
    """
    ...
    lr2s = np.array([(N - i) * step_val for i in range(1, N)], dtype=float)      # eta2 (matches index diagonal)
    ...
    for seed in seeds:
        ...
        # same data generation as your code (Gaussian)
        ...
        for idx in range(xs.shape[0]):
            ...
            # run GD up to max_steps; record at requested steps
...
def main():
    ...
    for s in steps_list:
        ...
        out_pdf = os.path.join(
            args.outdir,
            f"3-NN-diagonal-multicurve_step_{s}_lrmax_{lr_max_start}_{lr_max_end}_step_{lr_max_step}_n_{n}_d_{d}_h_{h}.pdf"
        )
        out_png = os.path.join(
            args.outdir,
            f"3-NN-diagonal-multicurve_step_{s}_lrmax_{lr_max_start}_{lr_max_end}_step_{lr_max_step}_n_{n}_d_{d}_h_{h}.png"
        )
