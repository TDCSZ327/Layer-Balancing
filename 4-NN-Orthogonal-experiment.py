...
def parse_args():
    p = argparse.ArgumentParser(
        description="4-layer linear NN: sweep lr_max and plot diagonal curves with a lr_max colorbar (arbitrary steps)"
    )
...
def train_step(W1, W2, W3, X, y, a, h, lr1, lr2, lr3):
    n = X.size(0)
    ...
    pred = inv_sqrt_h * X @ W1_ @ W2_ @ W3_ @ a
    ...
    return W1_new.detach(), W2_new.detach(), W3_new.detach(), loss.detach()
...
def compute_curves_for_one_lrmax(lr_max, *, d, h, n, test_size, device, seeds, steps_list):
    ...
    4-layer setting:
      pred = (1/sqrt(h)) * X @ W1 @ W2 @ W3 @ a

    We keep the plotting style close to the original code by using:
      lr1 = x-axis
      lr2 = lr3 = (lr_max - lr1) / 2
    ...
    for seed in seeds:
        ...
        # W1/W2/W3 orthogonal init; X orthogonal columns scaled
        ...
        a = torch.normal(mean=0, std=1/np.sqrt(h), size=(h, 1), device=device)                # h x 1, fixed last layer
        ...
        for k in range(xs.shape[0]):
            lr1 = float(xs[k])
            lr2 = float(lr_max - lr1) / 2.0
            lr3 = float(lr_max - lr1) / 2.0
            ...
            for target_step in steps_list:
                while t < target_step:
                    W1, W2, W3, _ = train_step(W1, W2, W3, X, y, a, h, lr1, lr2, lr3)
                    t += 1

                with torch.no_grad():
                    inv_sqrt_h = 1.0 / torch.sqrt(torch.tensor(h, dtype=W1.dtype, device=W1.device))
                    pred_test = inv_sqrt_h * x @ W1 @ W2 @ W3 @ a
                    resid = pred_test - xb
...
def main():
    ...
    for s in steps_list:
        ...
        cbar.set_label(r" $\eta_1+\eta_2+\eta_3$")
        ...
        out_pdf = os.path.join(
            args.outdir,
            f"4-NN-diagonal-multicurve_step_{s}_lr_n_{n}_d_{d}_h_{h}_lrmax_{lr_max_start}_{lr_max_end}_step_{lr_max_step}_plot.pdf"
        )
        out_png = os.path.join(
            args.outdir,
            f"4-NN-diagonal-multicurve_step_{s}_lr_n_{n}_d_{d}_h_{h}_lrmax_{lr_max_start}_{lr_max_end}_step_{lr_max_step}_plot.png"
        )
